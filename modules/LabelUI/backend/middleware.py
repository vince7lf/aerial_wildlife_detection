    ...
    , vascan_port, vascan_statut_repartition, tsn, coleo_category
        ...
        for r in result:
            ...
            for geojsonFile in geojsonFiles:
                if geojsonFile['geojsonFullFilepath'] == geojsonFullFilepath:
                    found = True
                    break
            if found: continue
            geojsonFiles.append({'path_filename': path_filename, 'splitted_path_filename': splitted_path_filename,
                                 'geojsonTemplateFullFilepath': geojsonTemplateFullFilepath,
                                 'geojsonFullFilepath': geojsonFullFilepath})

        server_host = self.config.getProperty('Server', 'static_host')
        server_port = self.config.getProperty('Server', 'static_port')
        ...
        for geojsonFile in geojsonFiles:
            ...
            with open(geojsonFile['geojsonTemplateFullFilepath'], 'r') as f:
                data = json.load(f)
                ...
                for feature in data['features']:
                    ...
                    for r in result:
                        ...
                        if (geojsonFile['path_filename'][0] == path_filename[0] and feature['properties']['Location'] ==
                            ...
                            annotation['url'] = 'http://' + server_host + ":" + server_port + '/' + '/'.join(
                                (project, 'files', str(r['image_path_filename'])))
                            feature['properties']['annotations'].append(annotation)

            # save the geojson
            with open(geojsonFile['geojsonFullFilepath'], 'w') as f:
                json.dump(data, f)
