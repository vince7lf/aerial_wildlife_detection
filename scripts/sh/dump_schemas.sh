...
# sudo -u postgres /bin/bash /app/aerial_wildlife_detection/scripts/sh/dump_schema.sh | sudo tee /var/log/dump_schema_$(date +\%Y\%m\%dT\%H\%M).log 2>&1
...
mapfile -t schemas < <(psql -d "$db_name" -t -c "$sql_query" |
...
for schema in "${schemas[@]}"; do
    ...
    pg_dump -Fc -d "$db_name" --schema="\"$schema\"" > "$output_dir/tes2-arbutus-$db_name-$schema-$timestamp.dump"
