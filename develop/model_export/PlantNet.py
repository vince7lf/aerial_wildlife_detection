'''
    Model export from ResNet18 to AIDE Model Marketplace.
...
from ai.models.detectron2.labels.torchvisionClassifier.resnet import ResNet18
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Exports a ResNet18 model for import in AIDE\'s Model Marketplace')
    parser.add_argument('--checkpoint', type=str,
                        help='Path of the ResNet18 model checkpoint to use.')
    ...
    # modify keys depending on whether state dict was created from resnet18 or resnet18.model
    keys = list(stateDict.keys())
    if keys[0].startswith('model.'):
        # created using the main resnet18 class; strip prefixes
        for key in keys:
    ...
    options = ResNet18.getDefaultOptions()

    cfg = get_cfg()
    # ResNet18._add_deepforest_config(cfg)  #TODO: hyperparameters
